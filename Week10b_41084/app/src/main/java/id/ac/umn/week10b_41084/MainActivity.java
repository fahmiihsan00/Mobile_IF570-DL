package id.ac.umn.week10b_41084;
...
public class MainActivity extends AppCompatActivity implements LoaderManager.LoaderCallbacks<Void>{
    ...
    public Loader<Void> onCreateLoader(int id, @Nullable Bundle args) {
        ...
        asyncTaskLoader.forceLoad(); return asyncTaskLoader;
    }
    ...
    static class ContohLoader extends AsyncTaskLoader<Void> {
        ...
        public ContohLoader(@NonNull Context context, int n, MainActivity main) {
            super(context); mCounter = n; mActivity = new WeakReference<MainActivity>(main);
        }
        @Nullable
        @Override
                    ...
                    if (mActivity.get() != null) {
                        mActivity.get().runOnUiThread(new Runnable() {
                            @Override public void run() {
                                if(progress < 100) { mActivity.get().mProgressBar .setProgress(progress);
